# ActLoRa (gateway-requests+stats side), based on TS-LoRa v1.1.11
                ...
                if (count == 3):  # the packet is a join request
                    ...
                    try:  # the packet may arrive corrupted
                        ...
                        try:
                            ...
                            print(str(dev_id)+":"+str(slot)+":"+mac+":"+str(JoinNonce[int(dev_id)])+":"+JoinEUI+":"+str(DevNonce))
                            wlan_s.send(str(dev_id)+":"+str(slot)+":"+mac+":"+str(JoinNonce[int(dev_id)])+":"+JoinEUI+":"+str(DevNonce))
                            msg = wlan_s.recv(512)
                            msg = msg.decode('utf-8')
                            try:
                                (rdev_id, acts, act_slots, DevAddr, AppSkey) = msg.split(":")
                                if (exists == 0) and (len(acts) > 0): # if the request is from an instigator
                                    actuators = acts.split(",")
                                    slots = act_slots.split(",")
                                    slots = list(map(int, slots))
                                    i = 0
                                    for a in actuators: # give to all actuators the correct slot
                                        registered.append([int(a), slot+slots[i]])
                                        i += 1
                                    index[req_sf] += max(slots)
                            except:
                                print("wrong net server packet format!")
                                pycom.rgbled(red)
                                wlan_s.close()
                            else:
                                try:
                    ...
                    try:
                        ...
                        (id, i, succeeded, dropped, rx, tx) = msg.split(":")
                        id = int(id)
                        i = int(i)
                        ...
                        print('Node %d: %d %d %d %f %f' % (id, i, succeeded, dropped, rx, tx))
                        # stats.append(dev_id)
                    pycom.rgbled(green)
