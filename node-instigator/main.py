# ActLoRa (instigator) based on TS-LoRa v1.1.11
...
def join_request(_sf):
    ...
    # slot generation
    while (len(DevAddr) < 8):
        DevAddr = "".join(["0", DevAddr])
    thash = uhashlib.sha256()
    thash.update(DevAddr)
...
def start_transmissions(_pkts):
    ...
    repeats = 0
    clock_correct = 0
    sync_slot = int(airtime_calc(my_sf,1,sack_bytes+2,my_bw_plain)*1000+guard+0)
    clocks = [sync_slot]
    print("-----")
    ...
    if (actuators%2 == 0):
        mod = actuators + 1
    succeeded = {}
    (active_rx, active_tx) = (0.0, 0.0)
    print("S T A R T")
    while(i <= _pkts): # stop after _pkts # of packets
        print(i, "----------------------------------------------------")
        chrono.reset()
        ...
        print("sleep time before transmission (ms):", t/1000)
        pycom.rgbled(off)
        time.sleep_us(t)
        ...
        print("sending ALARM of", len(pkg), "bytes at (ms):", (chrono.read_us()-start)/1000, "-------->")
        lora_sock.send(pkg)
        pycom.rgbled(off)
        active_tx += (chrono.read_us() - on_time)
        t = int((my_slot+1)*(2*guard+airt) - guard/2 - chrono.read_us())
        if (t < 0):
            t = 0
        print("sleep time before act acks (ms):", t/1000)
        time.sleep_us(t)
        # start of ack recv
        ack_time = chrono.read_us()
        print("starting ack reception at (ms):", ack_time/1000)
        lora_sock.setblocking(False)
        lora.init(mode=LoRa.LORA, rx_iq=True, region=LoRa.EU868, frequency=freqs[my_sf-5], power_mode=LoRa.ALWAYS_ON, bandwidth=my_bw, sf=my_sf)
        pycom.rgbled(white)
        while ((chrono.read_us() - ack_time) < (3*(2*guard+airt)-guard)): # receive mode for mod number of slots
            ...
            if (len(recv_pkg) > 2):
                ...
                if (int(recv_pkg_id) > 10):
                    ...
                    if (msg == "1"): # format check
                        if (dev_id not in act_acks):
                            act_acks.append(dev_id)
                            if int(dev_id) in succeeded:
                                succeeded[int(dev_id)] += 1
                            else:
                                succeeded[int(dev_id)] = 1
                        print("Received ack from", dev_id, "<---------")
                        # print(dev_id, lora.stats())
        if (len(act_acks) == actuators):
            i += 1
            act_acks = []
        elif (len(act_acks) < actuators) and (int(index) > 1):
            ack_repeats += 1
            if (ack_repeats == 3):
                dropped += 1
                ack_repeats = 0
                i += 1
                act_acks = []
        ack_lasted = chrono.read_us()-ack_time
        print("act ACKs lasted (ms):", ack_lasted/1000)
        active_rx += ack_lasted
        # end of ack recv
        pycom.rgbled(off)
        lora.power_mode(LoRa.SLEEP)
        ...
        lora_sock.settimeout(70/1000) # hardcoded, I'll fix it later
        try:
            while (rec == 0):
                machine.idle()
                sack_rcv = chrono.read_us()
                ...
                if (len(recv_pkg) > 2):
                    ...
                    if (int(recv_pkg_id) == (my_sf-5)):
                        ...
                        if (wt != guard) and (fpas == 0):
                            clock_correct = wt - guard
                        # print("clock correction for the next round (ms):", clock_correct/1000)
                        dev_id, leng, s_msg = struct.unpack(_LORA_RCV_PKG_FORMAT % recv_pkg_len, recv_pkg)
                        s_msg = s_msg.decode('utf-8')
                        (index, proc_gw, acks) = s_msg.split(":")
                        (index, proc_gw) = (int(index), int(proc_gw)*1000)
    ...
    stat_msg = ""
    for id in succeeded:
        # format = id:unique_pseq:succeeded:dropped:active_rx:active_tx
        if (id == MY_ID):
            stat_msg = str(id)+":"+str(i-1)+":0:"+str(dropped)+":"+str(active_rx/1e6)+":"+str(active_tx/1e6)
        else:
            stat_msg = str(id)+":"+str(i-1)+":"+str(succeeded[id])+":0:0.0:0.0"
...
# uncomment those two lines if you don't want to use the init_exp.py script (additional changes on gw_req may be needed)
# chrono.start()
